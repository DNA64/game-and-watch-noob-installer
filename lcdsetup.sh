...
# Install Inkscape (PPA for stable, Build from source for interim)
function gnw_install_inkscape(){
    ...
    if [[ " ${STABLE_RELEASES[@]} " =~ " ${UBUNTU_CODENAME} " ]]; then
        log INFO "Detected stable release: ${UBUNTU_CODENAME}"
        log INFO "Installing Inkscape from PPA..."
        ensure_command wget wget
        sudo add-apt-repository -y ppa:inkscape.dev/stable-1.1
        sudo apt-get update
        sudo apt install -y inkscape
    else
        log INFO "Detected interim release: ${UBUNTU_CODENAME}"
...
function lcd_game_shrinker(){
    echo ""
    echo "Do you want to install bzhxx's LCD-Game-Shrinker and its dependencies?"
